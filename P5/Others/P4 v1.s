...
.text #Directiva que indica la zona de codigo.
  ...
  invertirFilaLoad:
    #Cargo los valores de matrix.
    sub   $t1, $t1, $s2
    lw    $t5, 0($t1) #Guardo los valores en $t5
    
    #Almaceno los valores en aux
    sw    $t5, 0($t6) #Guardo los valores en $t6
    add   $t6, $t6, $s2
    
    addi  $t2, $t2, -1
    ble   $t3, $t2, invertirFilaLoad
  ...
  invertirFilaWrite:
    #Cargo los valores de aux.
    lw    $t5, 0($t6) #Guardo los valores en $t5
    add   $t6, $t6, $s2
    
    #Almaceno los valores en matrix.
    sw    $t5, 0($t4) #Guardo los valores en $t4
    add   $t4, $t4, $s2
    
    addi  $t2, $t2, -1
    ble   $t3, $t2, invertirFilaWrite
    j mostrarMatriz
  ...
  invertirColumnaWrite:
    ...
    addi  $t2, $t2, -1
    ble   $t3, $t2, invertirColumnaWrite
    j mostrarMatriz
