...
.data                         #Directiva que indica la zona de datos.
  ...
  cad12:.asciiz "Ha ocurrido un problema inesperado.\nPor favor, vuelva a cargar el programa.\n\n"
  space:.asciiz " "
  endl: .asciiz "\n"
...
.text                         #Directiva que indica la zona de codigo.
  ...
  invertirFila:
    ...
    mul  $t1, $t0, $s4
    mul  $t1, $t1, $s2
    add  $t1, $t1, $s0        #Sumo a la direccion base el desplazamiento.
    ...
    #Preparo los registros.
    li   $t0, 2
    div  $t0, $s4, $t0        #Numero de repeticiones.
    li   $t5, 0
    
  invertirFila0:
    beq  $t5, $t0, mostrarMatriz
    sub  $t1, $t1, $s2
    lw   $t2, 0($t1)          #Cargo en los registros el valor de la dir.
    lw   $t3, 0($t4)
    
    sw   $t3, 0($t1)          #Almaceno en la dir el contenido del registro.
    sw   $t2, 0($t4)
